...
This runbook walks through configuring an Auth0 tenant so that MCP clients such as Claude can authenticate to the Hermetiq MCP server (`https://mcp.<your-domain>/mcp`) via OAuth **Dynamic Client Registration (DCR)**. The Hermetiq chart side is covered in the Hermetiq chart README under [MCP Authentication](../charts/hermetiq/README.md#mcp-authentication); this document is only the IdP side.
...
URL, exactly as the client requests it. Use the `/mcp` endpoint URL:
```bash
auth0 api post resource-servers --data '{
  "name":"Hermetiq MCP","identifier":"https://mcp.<your-domain>/mcp",
...
auth0 api post client-grants --data '{
  ...
  "audience":"https://mcp.<your-domain>/mcp","scope":[] }'
```

**Important: use the `/mcp` URL everywhere.** Auth0 matches the `resource` a
client requests against API identifiers byte for byte, so
`https://mcp.<your-domain>` and `https://mcp.<your-domain>/` are different APIs.
MCP clients disagree about that trailing slash on a bare origin: some normalize
it in, others send the URL exactly as the server advertises it. Every client
sends `https://mcp.<your-domain>/mcp` identically, so standardize on it:

- MCP server URL in Claude (and other MCP clients): `https://mcp.<your-domain>/mcp`
- Auth0 API identifier: `https://mcp.<your-domain>/mcp`
- Auth0 default client-grant audience: `https://mcp.<your-domain>/mcp`
- Hermetiq values: leave `api.mcpResourceUrl` unset. It is the MCP *origin*; the
  server derives the `/mcp` resource from it, and the chart accepts tokens for
  both. The chart refuses a value with a path such as `/mcp`.
...
```bash
curl -fsS https://mcp.<your-domain>/.well-known/oauth-protected-resource/mcp | jq .resource
```
...
| `Service not found: https://mcp.<your-domain>…` (at `/authorize`) | Auth0 could not find an API whose identifier exactly matches the `resource` value requested by the client; the error names that value. The usual cause is a client pointed at the bare origin (`https://mcp.<your-domain>` or `https://mcp.<your-domain>/`) instead of `/mcp`. | Use `https://mcp.<your-domain>/mcp` for the Claude MCP URL, the Auth0 API identifier, and the client-grant `audience`, and leave `api.mcpResourceUrl` unset. Remove/re-add the Claude connector after changing the URL so it performs a fresh DCR flow. |
...
```bash
# Auth0 APIs. Confirm the MCP API identifier is exactly https://mcp.<your-domain>/mcp.
...
curl -fsS https://mcp.<your-domain>/.well-known/oauth-protected-resource/mcp | jq .
```

**Moving from the bare-origin setup.** Earlier versions of this runbook
registered the bare origin `https://mcp.<your-domain>/` and pinned
`api.mcpResourceUrl` to it. The chart still accepts tokens for the origin, but
the bare origin is exactly where clients disagree about the trailing slash. To
move, run Steps 2 and 3 with the `/mcp` URL, remove the `api.mcpResourceUrl`
pin, and remove/re-add the Claude connector with the `/mcp` URL.
